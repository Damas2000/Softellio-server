...
describe('DomainsService', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ...
                {
                    provide: dns_verification_util_1.DnsVerificationUtil,
        ...
        dnsUtil = module.get(dns_verification_util_1.DnsVerificationUtil);
        jest.clearAllMocks();
    });
    describe('create', () => {
        ...
        it('should create a system domain with auto-verification', async () => {
            const systemDomainDto = { ...createDomainDto, type: client_1.DomainType.SYSTEM };
            const systemDomain = { ...mockTenantDomain, type: client_1.DomainType.SYSTEM, isVerified: true, verificationToken: null };
        ...
        it('should throw BadRequestException for invalid domain format', async () => {
            ...
            await expect(service.create(1, createDomainDto)).rejects.toThrow(common_1.BadRequestException);
            expect(dnsUtil.validateDomainFormat).toHaveBeenCalledWith('example.com');
        });
        it('should throw ConflictException for existing domain', async () => {
            ...
            await expect(service.create(1, createDomainDto)).rejects.toThrow(common_1.ConflictException);
        });
    ...
    describe('update', () => {
        ...
        it('should throw BadRequestException for unverified custom domain set as primary', async () => {
            await expect(service.update(1, 1, updateDomainDto)).rejects.toThrow(common_1.BadRequestException);
        });
        it('should throw NotFoundException if domain not found', async () => {
            prismaService.tenantDomain.findFirst.mockResolvedValue(null);
            await expect(service.update(1, 1, updateDomainDto)).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('verify', () => {
        beforeEach(() => {
        ...
        it('should return verified status for system domain', async () => {
            const systemDomain = { ...mockTenantDomain, type: client_1.DomainType.SYSTEM };
        ...
        it('should throw NotFoundException if domain not found', async () => {
            prismaService.tenantDomain.findFirst.mockResolvedValue(null);
            await expect(service.verify(1, 1)).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('remove', () => {
        beforeEach(() => {
        ...
        it('should throw BadRequestException for last active domain', async () => {
            ...
            await expect(service.remove(1, 1)).rejects.toThrow(common_1.BadRequestException);
        });
