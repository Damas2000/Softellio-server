...
describe('PagesService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw BadRequestException for empty translations', async () => {
      ...
      await expect(service.create(emptyDto, 5)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw ConflictException for duplicate slug', async () => {
      ...
      await expect(service.create(mockCreatePageDto, 5)).rejects.toThrow(ConflictException);
      await expect(service.create(mockCreatePageDto, 5)).rejects.toThrow(
        "A page with slug 'test-sayfa' already exists in language 'tr'"
      );
    });
    ...
    it('should throw BadRequestException for invalid slug format', async () => {
      ...
      await expect(service.create(invalidSlugDto, 5)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException for duplicate languages', async () => {
      const duplicateLanguageDto = {
        ...
        translations: [
          { ...mockCreatePageDto.translations[0] },
          { ...mockCreatePageDto.translations[0] }, // Same language
        ],
      };

      await expect(service.create(duplicateLanguageDto, 5)).rejects.toThrow(BadRequestException);
    });
  ...
  describe('findAll', () => {
    ...
    it('should filter by status', async () => {
      const query: PageQueryDto = { status: PageStatus.PUBLISHED };
    ...
    it('should filter by language', async () => {
      const query: PageQueryDto = { language: 'tr' };
    ...
    it('should search in title and slug', async () => {
      const query: PageQueryDto = { search: 'test' };
  ...
  describe('findBySlug', () => {
    ...
    it('should throw NotFoundException for non-existent slug', async () => {
      ...
      await expect(service.findBySlug('non-existent', 'tr', 5, false)).rejects.toThrow(NotFoundException);
    });
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException for non-existent page', async () => {
      ...
      await expect(service.update(999, updateDto, 5)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should validate slug conflicts during update', async () => {
      ...
      mockPrismaService.pageTranslation.findFirst.mockResolvedValue({
        id: 999,
        pageId: 2, // Different page
        page: { id: 2 },
      });

      await expect(service.update(1, conflictDto, 5)).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('duplicate', () => {
    ...
    it('should throw NotFoundException for non-existent page', async () => {
      ...
      await expect(service.duplicate(999, 5)).rejects.toThrow(NotFoundException);
    });
  });
