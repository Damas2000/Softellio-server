...
async function ensureDemoTenant(prisma, result) {
...
async function repairSuperAdmin(authService, prisma, result) {
    const targetEmail = 'admin@softellio.com';
    const targetPassword = 'SuperAdmin123!';
    const targetRole = client_1.Role.SUPER_ADMIN;
    ...
    if (!superAdmin) {
        ...
        const needsUpdate = !superAdmin.isActive ||
...
async function repairTenantAdmin(authService, prisma, demoTenantId, result) {
    const targetEmail = 'admin@demo.softellio.com';
    const targetPassword = 'TenantAdmin123!';
    const targetRole = client_1.Role.TENANT_ADMIN;
    ...
    if (!tenantAdmin) {
        ...
        const needsUpdate = !tenantAdmin.isActive ||
...
bootstrap();
//# sourceMappingURL=auth-repair.js.map