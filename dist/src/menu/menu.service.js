...
let MenuService = class MenuService {
    ...
    async reorderMenuItems(menuId, tenantId, itemOrders) {
        ...
        if (existingItems.length !== itemIds.length) {
            throw new common_1.BadRequestException('One or more menu items not found or do not belong to this menu');
        }
        ...
        if (parentIds.length > 0) {
            ...
            if (existingParents.length !== new Set(parentIds).size) {
                throw new common_1.BadRequestException('One or more parent items not found or do not belong to this menu');
            }
        }
        await this.prisma.$transaction(itemOrders.map(({ id, order, parentId }) => this.prisma.menuItem.update({
            where: { id },
            data: {
                order,
